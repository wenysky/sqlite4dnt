    ...
    public partial class DataProvider : IDataProvider
    {
        ...
        public DataTable GetWebSiteAggForumTopicList(string showtype, int topnumber)
        {
            ...
            switch (showtype)
            {
                default://按版块查
                    {
                        topicList = DbHelper.ExecuteDataset("SELECT f.[fid], f.[name], f.[lasttid] AS [tid], f.[lasttitle] AS [title] , f.[lastposterid] AS [posterid], f.[lastposter] AS [poster], f.[lastpost] AS [postdatetime], t.[views], t.[replies], ff.[rewritename] AS [rewritename] FROM [" + BaseConfigs.GetTablePrefix + "forums] f LEFT JOIN [" + BaseConfigs.GetTablePrefix + "topics] t  ON f.[lasttid] = t.[tid] LEFT OUTER JOIN [" + BaseConfigs.GetTablePrefix + "forumfields] ff ON ff.[fid] = f.[fid] WHERE f.[status]=1 AND f.[layer]> 0 AND  ff.[password] ='' AND t.[displayorder]>=0").Tables[0]; break;
                    }
                case "1"://按最新主题查
                    {
                        topicList = DbHelper.ExecuteDataset("SELECT TOP " + topnumber + " t.[tid], t.[title], t.[postdatetime], t.[poster], t.[posterid], t.[fid], t.[views], t.[replies], f.[name], ff.[rewritename] AS [rewritename] FROM [" + BaseConfigs.GetTablePrefix + "topics] t LEFT OUTER JOIN [" + BaseConfigs.GetTablePrefix + "forums] f ON t.[fid] = f.[fid] LEFT OUTER JOIN [" + BaseConfigs.GetTablePrefix + "forumfields] ff ON ff.[fid] = f.[fid] WHERE t.[displayorder]>=0 AND f.[status]=1 AND f.[layer]> 0 AND ff.[password] ='' ORDER BY t.[postdatetime] DESC").Tables[0]; break;
                    }
                case "2"://按精华主题查
                    {
                        topicList = DbHelper.ExecuteDataset("SELECT TOP " + topnumber + " t.[tid], t.[title], t.[postdatetime], t.[poster], t.[posterid], t.[fid], t.[views], t.[replies], f.[name], ff.[rewritename] AS [rewritename] FROM [" + BaseConfigs.GetTablePrefix + "topics] t LEFT OUTER JOIN [" + BaseConfigs.GetTablePrefix + "forums] f ON t.[fid] = f.[fid] LEFT OUTER JOIN [" + BaseConfigs.GetTablePrefix + "forumfields] ff ON ff.[fid] = f.[fid] WHERE t.[digest] >0 AND f.[status]=1 AND f.[layer]> 0 AND ff.[password] ='' ORDER BY t.[digest] DESC").Tables[0]; break;
                    }
                case "3"://按版块查
                    {
                        topicList = DbHelper.ExecuteDataset("SELECT f.[fid], f.[name], f.[lasttid] AS [tid], f.[lasttitle] AS [title] , f.[lastposterid] AS [posterid], f.[lastposter] AS [poster], f.[lastpost] AS [postdatetime], t.[views], t.[replies], ff.[rewritename] AS [rewritename] FROM [" + BaseConfigs.GetTablePrefix + "forums] f LEFT JOIN [" + BaseConfigs.GetTablePrefix + "topics] t  ON f.[lasttid] = t.[tid] LEFT OUTER JOIN [" + BaseConfigs.GetTablePrefix + "forumfields] ff ON ff.[fid] = f.[fid] WHERE f.[status]=1 AND f.[layer]> 0 AND ff.[password] ='' AND t.[displayorder]>=0").Tables[0]; break;
        ...
        public DataTable GetWebSiteAggHotForumList(int topnumber)
        {
            ...
            string commandText = string.Format("SELECT TOP {0} f.[fid], f.[name], f.[topics],ff.[rewritename] FROM [{1}forums] f LEFT JOIN [{1}forumfields] ff ON f.[fid]=ff.[fid] WHERE f.[status]=1 AND f.[layer]> 0 AND ff.[password]='' ORDER BY f.[topics] DESC, f.[posts] DESC, f.[todayposts] DESC", topnumber, BaseConfigs.GetTablePrefix);
